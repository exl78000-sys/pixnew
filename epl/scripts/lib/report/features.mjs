...
/* ── 模擬賽後 ────────────────────────────────
 *
 * 跟上面兩種的差別是:這一篇講的**不是真的發生過的事**。
 * 它把 matchsim 產出的那一場攤成敘述,讓讀者對賽前機率有個畫面。
 *
 * 但它走的是同一條管線 —— facts → 模板 → verify。理由:模擬的數字更需要
 * 被綁住。真實比賽寫錯數字讀者可能會發現,模擬的寫錯沒有人查得出來,
 * 那才是真正危險的地方。所以規則一模一樣:文章裡的每個數字都要在 facts 裡。
 */
export function simBundle({ fixture, sim, diag = [], home, away, asOf, seasonLabel }) {
  const facts = [];
  const F = (...a) => facts.push(fact(...a));
  const S = sim.stats;

  F('score.home', `${home.en} 模擬進球`, sim.score.home);
  F('score.away', `${away.en} 模擬進球`, sim.score.away);
  F('xg.home', `${home.en} 本場期望進球`, round(sim.calibration.xgHome, 2));
  F('xg.away', `${away.en} 本場期望進球`, round(sim.calibration.xgAway, 2));
  for (const [key, side, team] of [['home', S.home, home], ['away', S.away, away]]) {
    F(`${key}.shots`, `${team.en} 射門`, side.shots);
    F(`${key}.onTarget`, `${team.en} 射正`, side.onTarget);
    F(`${key}.corners`, `${team.en} 角球`, side.corners);
    F(`${key}.fouls`, `${team.en} 犯規`, side.fouls);
    F(`${key}.yellow`, `${team.en} 黃牌`, side.yellow);
    F(`${key}.possession`, `${team.en} 控球率`, side.possession, `${side.possession}%`);
  }

  // 時間軸裡的進球:每一顆都要有 fact,否則模板引用分鐘數會被 verify 擋下
  const goals = (sim.timeline ?? []).filter(t => t.kind === 'goal');
  goals.forEach((g, i) => F(`goal.${i}.minute`, `第 ${i + 1} 顆進球的時間`, g.minute, `${g.minute}`));

  /* 戰術判斷句子裡的每個數字也要進 facts,否則模板引用它們就過不了 verify。
     matchdiag 的 evidence 本來就是為了這件事而存在 —— 這裡把它攤平成 facts。 */
  diag.forEach((d, i) => {
    for (const [label, value] of Object.entries(d.evidence ?? {})) {
      if (typeof value !== 'number') continue;
      F(`diag.${i}.${label}`, `${d.side === 'home' ? home.en : away.en} ${label}`, value);
    }
  });

  const p = fixture?.prediction;
  if (p) {
    F('pre.home', '賽前主勝機率', p.home, pctText(p.home));
    F('pre.draw', '賽前和局機率', p.draw, pctText(p.draw));
    F('pre.away', '賽前客勝機率', p.away, pctText(p.away));
  }

  return {
    kind: 'sim',
    key: fixture?.id ?? `${home.code}|${away.code}`,
    seed: sim.seed ?? null,
    home: { code: home.code, en: home.en, zh: home.zh },
    away: { code: away.code, en: away.en, zh: away.zh },
    goals: goals.map(g => ({ minute: g.minute, side: g.side, player: g.player, assist: g.assist })),
    // 戰術判斷直接沿用 matchdiag 的結果,不在這裡重算 —— 兩邊會漸漸對不上
    diag,
    facts,
    provenance: {
      source: '本站事件模擬引擎(matchsim)',
      model: '控球鏈事件狀態機,全場期望進球受賽前 Dixon-Coles λ 約束',
      asOf, seasonLabel,
    },
  };
}
