...
/* 牌、換人與半場標記。**進球不在這裡** —— 它由 goalsOf 用「比分變了」判定,
   ...
   2. **沒見過的代碼不分類。** 目前見過的是 Y、R、YR,**每一種都是核對過才放行的**:
      - `R`(直接紅牌):2026-27 第 1 輪 BHA vs AVL 第 40 分有一筆,拿 FPL 的逐球員資料
        獨立核對 —— FPL 說 AVL 的 Gomes 紅牌 1、上場 39 分,兩邊指的是同一個人同一件事。
      - `YR`(**兩黃變紅**):2026-27 第 4 輪 SUN vs ARS 90+6 分出現,而**同一個人(63733)
        28 分鐘已經吃過一張 `Y`** —— 那就是第二張黃。獨立核對用下面那條 FPL 的記法差異:
        官方這一隊是 **5 張 Y + 1 張 YR**,FPL 是 **4 張黃 + 1 張紅**,差的正好是被罰下那人的黃牌。
        跟當初確認 `R` 完全同一個方法(2026-09-12 核對)。
      第四種代碼出現時 kind 會是 null、原碼留在 kindRaw,測試會紅,先核對過才放行。
...
/* YR 是**第二張黃牌造成的紅牌**,不是另一種黃牌 —— 畫面上要讓讀者一眼知道這人下場了。
   標成「兩黃罰下」而不是「紅牌」,因為對讀者(與對 FPL 的對帳)那是不同的事:
   FPL 把這人記成「黃 0 紅 1」,所以逐隊黃牌數會比官方少一張。 */
export const CARD_KINDS = { Y: '黃牌', R: '紅牌', YR: '兩黃罰下' };
// 會讓球員離場的牌(畫面與統計都要分得出來)
export const CARD_SENDS_OFF = new Set(['R', 'YR']);
