#!/usr/bin/env node
/* 抓法甲球員的整季數據(Understat)。實作在 `lib/understat-fetch.mjs` —— 跟西甲德甲**同一支**。
 *
 *   npm run fr1:players            # 只補還沒有的賽季
 *   npm run fr1:players -- --force # 重抓
 *
 * ── 聯賽代號是**實測**來的,不是猜的 ──
 * `probe-new-leagues.mjs` 在 CI runner 上跑過(2026-09-15):
 *   - 代號是 `Ligue_1`(候選逐一試出來的;猜錯的話端點回 200 + error 物件或空陣列,
 *     不會拋錯 —— 英冠那次四種寫法全回空陣列)
 *   - 欄位跟西甲那一組**一個不少**,所以下游照用
 *   - 2025-26 回 553 筆
 *
 * 抓不到的原因只有一個:開發沙箱的出口代理不放行 understat.com(實測 CONNECT 403)。
 * 在 CI runner 上跑同一支就會成功 —— 所以法甲的球員層是「還沒抓」,不是「拿不到」。
 *
...
await fetchUnderstatPlayers({
  ...
  dir: 'understat-ligue-1',
  seasons: [
    { label: '2025-26', provider: 2025 },
    { label: '2026-27', provider: 2026 },
  ],
  force: process.argv.includes('--force'),
  note: 'POST league=Ligue_1&season=YYYY，整季一個請求。無背號、無頭貼、無傷停、無出生日期。',
});
