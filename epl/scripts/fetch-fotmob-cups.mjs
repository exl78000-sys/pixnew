...
const MAX_DETAILS = Number(arg('max-details') ?? 16);
...
async function main() {
  ...
  for (const cup of FOTMOB_CUPS) {
    ...
    const seasonsOut = [];
    const missing = [];
    let available = prev?.availableSeasons ?? null;
    ...
    for (const label of WANT) {
      ...
      if (old && !FORCE && age < ttl) {
        ...
        if (Array.isArray(body.allAvailableSeasons)) available = body.allAvailableSeasons;
      ...
      /* PK 場的比數與勝方:單場詳情。快取仍新鮮也要補(不然上季 36 場 PK 要等 7 天一批)。
         掛上去前核對隊 id 與最終比分 —— 對不上就是抓錯場,寧可留「勝方待查」。 */
      ...
      for (const m of pending) {
        ...
        try {
          ...
          if (!parsed) { bad++; console.log(`    ✗ 詳情 ${m.id}:沒有 header.status/teams`); continue; }
          if (parsed.homeId !== m.home?.sourceId || parsed.awayId !== m.away?.sourceId) { bad++; console.log(`    ✗ 詳情 ${m.id}:隊 id 對不上賽程端點(${parsed.homeId}/${parsed.awayId} vs ${m.home?.sourceId}/${m.away?.sourceId}),不掛`); continue; }
          if (parsed.score && m.final && (parsed.score[0] !== m.final[0] || parsed.score[1] !== m.final[1])) { bad++; console.log(`    ✗ 詳情 ${m.id}:最終比分對不上(${parsed.score.join('-')} vs ${m.final.join('-')}),不掛`); continue; }
          if (parsed.conflict) { bad++; console.log(`    ✗ 詳情 ${m.id}:PK 比數與輸家對不起來,不掛`); continue; }
          if (!parsed.pens && !parsed.pensWinner) { bad++; console.log(`    · 詳情 ${m.id}:沒有 PK 比數也沒有輸家(${m.home?.name} v ${m.away?.name})`); continue; }
          ...
          got++;
        } catch (e) { bad++; console.log(`    ✗ 詳情 ${m.id}:${e.message}`); }
      }
      if (got) season.matches = season.matches.map(m => withCupDetails(m, season.details?.[m.id] ?? null));
      ...
      if (cc) console.log(`    核對 SportMonks:對上 ${cc.matched}/${cc.compared}・一致 ${cc.agree}・不一致 ${cc.disagree.length}・無法核對 ${cc.unverified}`
