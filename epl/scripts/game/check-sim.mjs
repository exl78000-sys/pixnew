...
if (simShots && realShots) {
     ...
     兩個獨立來源核對過:shotmap 逐顆算的這一場兩隊射正率 32.1%、CSV 的 stf ÷ sf 31.6%(鐵則五)。 */
  {
       ...
       一樣好、而 0~5 m 那一帶曝光是 0。只印不判 —— 這是 5c 的驗收條件,不是現在的紅線。 */
    {
      ...
      /* **撲搶的天花板**(2026-09-19,階段 5d)。5c 的結論是「缺的全是曝光」,
         而最順手的下一步是給防守員一個「撲上去擋」的動作。動手之前先回推它的上限
         (4k 的規矩:先問模型的函式族生不生得出目標的形狀):
         `laneReach` = 這一腳射出去的時候,**有人來得及**撲進 `DEFLECT_R`
         (球滾地會減速所以飛行時間解二次式;人這一側取「已經在全速」的樂觀上界)。
         **就算撲搶百發百中,這一帶也只能擋到這個數字。**

         量出來的形狀是決定性的:天花板**隨離門距離單調上升**(18/25/32/25/35/46/66),
         而真實的封阻率是**駝峰**(聯盟 8/17/29/39/39/32/20)——
         15~25 公尺那兩帶**天花板比真實還低**,而 30 公尺外**高到三倍**。
         沒有任何「撲搶成功率」生得出駝峰:調高就把遠射擋爆,調低就補不上中距離
         (**2026-09-21 階段 5l 把這個否定收窄了**:形狀確實生不出來,但使用者把遊戲側的
          驗收改成「畫面上的行為像不像踢球」之後,這個動作就值得做 —— `BLOCK_REACH` 接上去
          封阻 2.9% → 4.8%,兩組獨立種子都複現。**這個天花板仍然是對的**,它說的是
          「就算百發百中也只能到這裡」,而 5l 量到實際更低的原因:整段飛行裡最近的防守者
          離飛行線 5.66 公尺,連撲都撲不到 —— 那是**站位**,不是這個動作)。
         根因看得見:`4 m 內有人` 也是單調上升(27 → 68%)——**本站從遠距離往人堆裡射**,
         而扣扳機那一行沒有任何「路上有沒有人」的項。 */
      const lf = k => mean(rows.map(r => r.st.counts.laneFar?.[k] ?? 0));
      const lr = k => mean(rows.map(r => r.st.counts.laneReach?.[k] ?? 0));
      const far = [], rch = [];
      for (let k = 0; k < 7; k++) {
        far.push(pc(ls(k) > 0 ? lf(k) / ls(k) : null));
        rch.push(pc(ls(k) > 0 ? lr(k) / ls(k) : null));
      }
      ...
      }
      {
        ...
        /* **撲球的診斷**(2026-09-21,階段 5l)。封阻只到 4.8% 的時候,要分得出
           「撲了但差一點」與「根本到不了」—— 兩個要修的地方完全不同(前者調判定半徑,
           後者是加速度 / 起步時機的問題)。`minPerp` 是**整段飛行**裡最近的防守者
           離飛行線最近到幾公尺,而「路上有人」那一排是**射門那一瞬間**的,兩個不一樣。 */
        {
          const sc = rows.reduce((a2, r) => {
            const c2 = r.st.counts.shotChase ?? {};
            for (const [k, v] of Object.entries(c2)) if (typeof v === 'number') a2[k] = (a2[k] ?? 0) + v;
            return a2;
          }, {});
          const sh = Math.max(1, sc.shots ?? 0);
          line('　撲球:一腳射門有幾個人撲',
            `${((sc.tries ?? 0) / sh).toFixed(2)} 人`,
            `有人撲的佔 ${(100 * (sc.withTry ?? 0) / sh).toFixed(0)}%・`
            + `整段飛行最近的防守者離飛行線 **${((sc.minPerp ?? 0) / sh).toFixed(2)} m**`
            + `(擋得到要 < ${DEFLECT_R_DOC ?? 0.75} m)・`
            + `差一點(${DEFLECT_R_DOC ?? 0.75}~1.5 m)的佔 ${(100 * (sc.near ?? 0) / sh).toFixed(0)}%`);
        }
        /* **回推「要對上真實得是多少」**(4k 的規矩:先問模型做不做得到,再決定要不要調)。
           封阻率 = 曝光 × 轉換,所以在現在這個轉換率下,要生出真實的封阻率
        ...
        line('　回推:要對上真實的封阻率,曝光得是',
          ...
          + `而本站實際是 ${S > 0 ? (100 * O / S).toFixed(1) : '—'}%`);
      }
      /* **扣扳機有沒有在挑空走廊**(2026-09-20,階段 5g)。5f 否定收窄、5d 否定撲搶之後
         剩下的候選:射手等到沒有人擋才出手。量在**每一個射程內的決策點**上,
         射與不射用同一份、同一條線,再看兩群差多少。

         **這幾個數字跟上面那幾排不可比**:上面用球**真正的飛行方向**(射出去才存在),
         這裡用射手 → **球門中心**(決策當下唯一看得到的意圖)——拿飛行方向分群
         等於用結局去條件化。5c 實測兩條線在 675 腳上各錯三分之一,所以分開印、分開讀。 */
      {
        const sum = a => a.reduce((x, y) => x + y, 0);
        const g = k => rows.reduce((a, r) => a + sum(r.st.counts[k]), 0);
        const N = g('decN'), O = g('decOcc'), F = g('decFar');
        const SN = g('decShot'), SO = g('decShotOcc'), SF = g('decShotFar');
        const nonN = N - SN, nonO = O - SO;
        const pc = (a, b) => (b ? `${(100 * a / b).toFixed(1)}%` : '—');
        const seOf = (a, b) => (b ? 100 * Math.sqrt((a / b) * (1 - a / b) / b) : null);
        const pS = SN ? SO / SN : null, pN = nonN ? nonO / nonN : null;
        const sed = (pS != null && pN != null)
          ? 100 * Math.sqrt(pS * (1 - pS) / SN + pN * (1 - pN) / nonN) : null;
        console.log('');
        console.log(`  ${'決策點的球道(意圖線,跟上面那幾排不可比)'.padEnd(26, '　')} `
          + `${N} 個決策點、${SN} 腳射門(出手率 ${pc(SN, N)})`);
        line('　路上有人:射的 / 沒射的',
          `${pc(SO, SN)} / ${pc(nonO, nonN)}`,
          sed ? `差 ${((pS - pN) * 100).toFixed(1)} 個百分點 = ${(((pS - pN) * 100) / sed).toFixed(1)} SE`
            + ` —— **接近 0 就代表扣扳機沒有在挑空走廊**(它本來就沒讀球道,只讀距離 / 張角 / 最近的對手)` : '—');
        line('　P(射|有人) / P(射|沒人)',
          `${pc(SO, O)} / ${pc(SN - SO, N - O)}`,
          `比值 ${O && N - O && SN - SO ? ((SO / O) / ((SN - SO) / (N - O))).toFixed(2) : '—'}`
          + ' —— 1.00 是完全不挑;低於 1 是稍微避開,而那是 `pressure` 那一項的副作用');
        /* **這一行才是 5g 真正的結論**:把扣扳機的選擇完全拿掉,曝光最多就是
           「全部決策點」那個值 —— 它是**這條路的天花板**(5d 的規矩:先算上限再決定要不要寫)。
