...
const CHECKS = [
     ...
     不要碰敘事裡描述過去事件的數字。** 後者是紀錄,不是狀態。 */
  {
    ...
    key: '歐冠有隊徽的球隊數',
    /* 2026-09-22 從紅線降成只回報。理由是**它的變動來源換了**:
       隊徽原本只有人工交付才會多(所以「只有人為改動才會變」成立),
       而現在 `npm run ucl:crests` 每次部署都會把缺的補上 ——
       這個分子從此會自己往上跑,寫成紅線的話補齊的那一天就紅在「補齊了」
       (本站在歐冠涵蓋率那條上踩過一次:斷言寫「一定要有球隊沒評分」)。
       分母(出現過幾支球隊)還是紅線 —— 那只有多一季才會變。 */
    drifts: true,
