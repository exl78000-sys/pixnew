...
export async function renderGame(app) {
  try {
    ...
    if (!GAME_LEAGUES.includes(lg)) {
      ...
      app.innerHTML = `<h1>模擬遊玩</h1><div class="note">模擬遊玩目前只有英超:它要<b>推估先發、主罰順位與逐人能力係數</b>,
        這三樣只有英超靠 FPL 拿得到,${C.esc(zh)}在本站接得到的來源裡沒有(不是還沒接)。
        <a href="${C.link('explore', { view: 'duel', league: 'pl' })}">切到英超玩</a></div>`;
      return;
    }
    ...
    function statsHtml() {
      ...
      return `${row('控球 %', ph ?? '—', ph != null ? 100 - ph : '—', `持球秒數・目標 ${pTarget != null ? `${Math.round(pTarget)}:${100 - Math.round(pTarget)}` : '—'}`)}${row('射門', H.shots, A.shots)}${row('射正', H.on, A.on)}${row('被封阻', H.blocked, A.blocked)}
        ...
        <div class="tiny dim" style="margin-top:6px">全部由<b>畫面上已經發生的事件</b>累計 —— 這個引擎不會算到未來,所以不會提前洩露還沒演的射門。
          ...
          <b>被封阻的射門仍然比真實少</b>(真實約三成,這裡不到一成)。
          兩件事是<b>遊戲變數</b>(不是從資料算出來的):逼搶的人<b>站在持球者與自家球門之間、跟著他退</b>
          (退的速度上限是「跑」,帶得快的人甩得掉他),以及球在飛的時候,離飛行線三公尺內的人會<b>撲上去擋</b>。
          拿來對的是射門那一瞬間的站位(StatsBomb 開放資料):最近的防守者站在射手與球門之間的,
          真實約五成、這裡約四分之一;球的路上有人,真實約五成、這裡一成多。
          剩下的差距還沒有拆清楚,已知不同的一項是射門的方式:真實有四成多是接到球就射,這裡的射門一律要先控住球。
    ...
    /* 底下「這張圖哪些是真的」原本是一整段近千字的牆,還夾著 npm 指令;
       2026-09-24 全站掃描時拆成清單,內容一項不少(測試釘著的那幾句原字保留)。 */
    app.innerHTML = `
      ...
      <div class="note" style="margin-top:12px"><b>這張圖哪些是真的。</b>
        <ul style="margin:6px 0 0;padding-left:18px;display:grid;gap:4px">
          <li><b>真資料</b>:名單、背號、角色、陣型選項、<b>逐人最高速度</b>(FotMob,當成跑速上限)、
            兩隊的跑動節奏、事件率(射門 / 角球 / 犯規 / 黃牌 / 越位)、控球分布、壓迫強度
            (每 100 次對手傳球的抄截 + 攔截),以及進球的錨 λ(站上對這一場的預測)。</li>
          <li><b>從模擬長出來的</b>:每一腳傳球、射門、角球、界外球、越位、犯規與牌 ——
            不是抽出來的,是球跟人在場上互動的結果。射門的離門距離與禁區內比例,
            對著本站倉庫裡的 FotMob 逐場射門座標校準過(差距會隨版本變,這一頁不抄數字)。</li>
          <li><b>遊戲規則</b>(沒有資料可以校準):加速度與煞車上限、控球半徑、逼搶距離、
            逼搶的人站在持球者與球門之間以及他退得多快(上限是「跑」)、
            折射的機率與角度、接球者的優勢、扣扳機的機率形狀、戰術指令每一級改多少。
            站位本身沒有追蹤資料可以校準,拿來對的是射門那一瞬間的站位(StatsBomb 開放資料)。</li>
          <li><b>體能</b>:跑過的路越多、跑得出來的速度越低(換上場的人從零算)。
            <b>衰退的幅度沒有真值可以校準</b>(逐場體能資料只有全場總計),常數取「不把其他對得上的數字弄歪」的最大值。
            它也<b>不產生真實世界的後段進球潮</b>(真實下半場進球多兩成,本站持平)——
            那是落後方壓上、生力軍與終場補時的事,不是體力。</li>
          <li><b>補時比真實的短</b>,不是 bug:這裡死球只等一兩秒,補時照這一半實際死掉的時間補回去。</li>
          <li><b>進球情境</b>只分運動戰 / 角球 / 快攻 / 界外球 / 定位球 / 十二碼;
            直接罰球射門與個人突破沒有可靠判準,不假裝分得出來。</li>
          <li><b>還沒做的</b>:直接紅牌(只做兩黃)、受傷、換人次數與窗口的規則;
            戰術的心態 / 寬度 / 節奏 / 直接度(引擎接得動,但量下去只有直接度改的是踢法、其餘三軸改的是強弱,所以都還沒掛上來)。
            <b>刻意不做</b>:自動換人(換人是玩家的決定)。沒做的一律不放欄位。</li>
          <li><b>跟真實管線只有一條關係</b>:沒有任何改動時,N 場的平均進球回得到站上的 λ
            (統計上相等,不是逐場相等);任何操作都不寫回資料,也不影響站上任何一頁。</li>
        </ul></div>
      ${C.foot(data.meta)}`;
    renderControls();
  } catch (e) {
