...
| **前端把聯賽的事實寫死** | `page-teams` 兩個分支都寫「20 支球隊」—— 英超西甲剛好都是 20,所以三年沒事;英冠 24 隊,照抄就在畫面上印一個假數字。同類的還有:sim 表的「前四」(英冠是前 2 直升、3~6 附加賽,第四名沒有意義)、`meta.edition === 'basic'` 被當成「是不是西甲」的二元旗標 | 數字一律從資料算(`teams.length`);分界線看資料有沒有那個欄位(`promotionPct`),不要在前端寫死聯賽代碼;文案由各聯賽的 build 寫進 `meta.intro` / `meta.boundaries` |
