...
**英冠只做球隊與比賽那一層**,沒有球員、xG、陣容、教練 ——
不是還沒做,是**實測過拿不到**(Understat 不涵蓋英冠,對照組與端點見
`docs/英冠-資料來源探測.md`)。導覽列只掛做得出來的三頁。
...
| **`npm test` 夾在 `laliga:build` 與 `build` 中間** | 資產戳是 `build` 最後一步(`stamp-assets`)寫的。測試卡在中間跑的話,es1 的 `meta.json` 剛被 `laliga:build` 重寫、戳還沒補回來 —— 「meta 記的戳跟實際檔案一致」**每次 CI 都假紅**,而 `continue-on-error: true` 讓沒有人去看 | 測試要驗的是**最終狀態**,所以排在 `build` 之後。2026-08-28 修正 `epl-live.yml`。這條在本機重現得出來:跑 `laliga:build` 之後直接 `npm test` 就會紅 |
...
| **「不是英超」不等於「就是西甲」** | 前端 `league()` 寫死 `=== 'es1' ? 'es1' : 'pl'` → `?league=en2` 靜靜退回英超,頁面正常顯示、只是顯示的是另一個聯賽的資料。vault 產生器兩處同病:`dataDir` 把非 pl 一律指向 `leagues/es1`、`clubDirectory` 非 pl 一律讀 `teams-la-liga.json` → vault 裡長出 `英冠/球隊/FC Barcelona.md` | 判斷一律走註冊表(`LEAGUES` / `lg.key`),不要用「是不是某一個」的二元式。這種寫法在只有兩個聯賽時看起來完全正確 |
...
| **只顯示一部分的清單,要用「單位」當上限而不是筆數** | 修上面那條時第一版想改成 `slice(0, perRound)`。但改期的比賽會讓最近的一場落在**更早的輪次**(一場延到十二月才踢的第 1 輪),那時候「只取第一場所屬的輪次」就把整個第 2 輪藏起來 —— 比原本的 bug 更糟 | 規則是「往後收,湊滿一輪的場數之後**在輪次交界處**停」:永遠不切一半,落單的改期場次也不吃掉整輪的名額。抽成 `core.js` 的 `countdownFixtures` 才測得到 —— **測試看不到 DOM,留在頁面裡就只能用正則掃原始碼**,那種測試守不住邏輯 |
